    ...
    <div :class="{ 'translate-x-0': open }" class="transform -translate-x-full lg:translate-x-0 transition-all duration-300 fixed top-0 start-0 bottom-0 z-[60] w-64 bg-white border-e border-gray-200 pt-7 pb-10 overflow-y-auto lg:block lg:end-auto lg:bottom-0 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-slate-700 dark:[&::-webkit-scrollbar-thumb]:bg-slate-500 dark:bg-gray-800 dark:border-gray-700">
        ...
        <nav class="p-6 w-full flex flex-col flex-wrap">
            <ul>
                ...
                <x-sidebar-nav-link :href="route('pilots.index')" :active="request()->routeIs('pilots.index')" icon="heroicon-o-truck" wire:navigate>
                    Pilotos
                </x-sidebar-nav-link>
                <x-sidebar-nav-link :href="route('agencies.index')" :active="request()->routeIs('agencies.index')" icon="heroicon-o-home" wire:navigate>
                    Agencias
                </x-sidebar-nav-link>
                <x-sidebar-nav-link :href="route('planning.index')" :active="request()->routeIs('planning.index')" icon="heroicon-o-folder" wire:navigate>
                    Planificación
                </x-sidebar-nav-link>
                <x-sidebar-nav-link :href="route('orders.index')" :active="request()->routeIs('orders.index')" icon="heroicon-o-inbox" wire:navigate>
                    Control de pedidos
                </x-sidebar-nav-link>
            </ul>
        </nav>
